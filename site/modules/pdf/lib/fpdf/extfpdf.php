...
class ExtFPDF extends FPDF
{
	...
	private $issetcolor;
	private $H1, $H2, $H3, $H4;
	private $tmpfontsize, $tmpfontstyle;
	...
	public function BookmarkUTF8($txt, $level = 0, $y = 0)
	{
	...
	public function WriteHTML($html, $x = -1, $y = -1, $w = -1, $h = -1)
	{
		if ($x > 0) {
			$lm = $this->lMargin;
			$this->lMargin = $x;
		...
		foreach ($a as $i => $e) {
			if ($i % 2 == 0) {
				...
				if ($this->HREF)
					...
					$this->Write(5, stripslashes(text_entities($e)));
			} else {
				//Tag
				if ($e[0] == '/')
