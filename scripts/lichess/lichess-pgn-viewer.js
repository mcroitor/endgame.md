var LichessPgnViewer = (function () {
    ...
    const parseComment = (comment) => {
        ...
        const shapes = [];
        const text = comment
            .replace(/\s?\[%(emt|clk)\s(\d{1,5}):(\d{1,2}):(\d{1,2}(?:\.\d{0,5})?)\]\s?/g, (_, annotation, hours, minutes, seconds) => {
            ...
            .replace(/\s?\[%eval\s(?:#([+-]?\d{1,5})|([+-]?(?:\d{1,5}\.?\d{0,5}|\.\d{1,5})))(?:,(\d{1,5}))?\]\s?/g, (_, mate, pawns, d) => {
